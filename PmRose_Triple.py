...
for pidx, staloc in enumerate(stalocs):
    ...
    ## Limit pressure scale due to gap in WCI data resulting in non-real pressure values ##
    if sta == 'WCI':
        limit = 35563
        pm = pm[cpress < limit]
        c = c[cpress < limit]
    ...
    ## Lettering ##
    fig.text(0.135, 0.76, '(a)', ha = 'center', va = 'center', rotation = 'horizontal', fontsize = 30, color='k')
    fig.text(0.400, 0.76, '(b)', ha = 'center', va = 'center', rotation = 'horizontal', fontsize = 30, color='k')
    fig.text(0.675, 0.76, '(c)', ha = 'center', va = 'center', rotation = 'horizontal', fontsize = 30, color='k')
...
plt.subplot(1,3,3)
plt.title(sta[2] + ' ' + loc[2], fontsize=25)

plt.savefig('PmRose_Triple_' + sta[0] + '_' + str(int(1/fmax)) + '-' + str(int(1/fmin)) + 's_' + str(stime.julday) + '-' + str(etime.julday) + '.png')
plt.show()
plt.clf()